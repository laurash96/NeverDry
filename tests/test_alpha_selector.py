...
unit is then looked up under ``selector.<key>.unit_of_measurement.<unit>``. Before
that release the selector's schema refuses the key outright, so the form would fail
to open on an install still inside the range ``hacs.json`` declares. The helper
therefore tries the key and falls back to the plain selector.
...
def test_omits_the_key_when_home_assistant_rejects_it(helper):
...
def test_both_paths_describe_the_same_box(helper):
    accepted = dict(helper(True).config)
    accepted.pop("translation_key")
    assert accepted == helper(False).config
...
def test_english_keeps_the_unit_the_fallback_shows(helper, catalogue):
    config = helper(True).config
    assert _translated_unit(catalogue, config) == config["unit_of_measurement"]
